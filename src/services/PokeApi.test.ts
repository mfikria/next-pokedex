...
describe('PokeApi', () => {
  ...
  describe('getPokemonList', () => {
    it('returns a list of Pokemon with the expected length', async () => {
      const mockResponse = {
        ok: true,
        json: async () => ({ results: [{ name: 'bulbasaur' }, { name: 'charmander' }, { name: 'squirtle' }] })
      };
    ...
    it('throws an error if the API request fails', async () => {
      ...
      await expect(pokeApi.getPokemonList({ limit: 20, offset: 1000 })).rejects.toThrow();
    });
  });
  ...
  describe('getPokemonDetail', () => {
    it('returns a Pokemon with the expected properties', async () => {
      const mockResponse = {
        ok: true,
        json: async () => ({ name: 'bulbasaur', types: [{ type: { name: 'grass' } }, { type: { name: 'poison' } }] })
      };
      mockFetch.mockResolvedValue(mockResponse);
    ...
    it('throws an error if the API request fails', async () => {
      ...
      await expect(pokeApi.getPokemonDetail('invalid-pokemon')).rejects.toThrow();
    });
  });
});
